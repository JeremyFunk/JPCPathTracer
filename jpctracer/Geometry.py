...
ptr_geometries_t = ct.POINTER(geometries_t)

ctracer.bvhtree_triangles_free.argtypes = [ptr_geometries_t]
ctracer.bvhtree_spheres_free.argtypes = [ptr_geometries_t]
ctracer.bvhtree_instances_free.argtypes = [ptr_geometries_t]

ctracer.bvhtree_triangles_build.argtypes = [ptr_geometries_t]
ctracer.bvhtree_spheres_build.argtypes = [ptr_geometries_t]
ctracer.bvhtree_instances_build.argtypes = [ptr_geometries_t]
...
class GeometryFactory:
    ...
    def __init__(self, material_factory: MaterialFactory):
        ...
        self._c_sphere_bvhtree = False
        self._c_tri_bvhtree = False
        self._c_instances_bvhtree = False
        self._matfac = material_factory
        self._c_geometries = ct.geometries_t()

    def spheres(self, positions, radii, material_slots):

        self._sphere_pos = np.concatenate(
            [self._sphere_pos, positions], dtype=np.float32)
        self._sphere_radii = np.concatenate(
            [self._sphere_radii, radii], dtype=np.float32)
    ...
    def triangles(self, vertices, uvs, normals, verticies_ids, normals_ids, uvs_ids, material_slot_ids):
        ...
        self._tri_normals = np.concatenate(
            [self._tri_normals, normals], dtype=np.float32)
        ...
        self._tri_uvs_ids = np.concatenate(
            [self._tri_uvs_ids, uvs_ids], dtype=np.uint32)
    ...
    def _free_bvhtrees(self):
        if(self._c_tri_bvhtree == True):
            ctracer.bvhtree_triangles_free(self._c_geometries)
            self._c_tri_bvhtree = False
        if(self._c_sphere_bvhtree == True):
            ctracer.bvhtree_spheres_free(self._c_geometries)
            self._c_sphere_bvhtree = False
        if(self._c_instances_bvhtree == True):
            ctracer.bvhtree_instances_free(self._c_geometries)
            self._c_instances_bvhtree = False
    ...
    def _build_geometries(self):
        ...
        if(self._state == "init"):
            ...
            tris.material_slots = self._tri_material_slot_ids.ctypes.data_as(
                uint_p)

            tris.mesh_count = len(self._tri_mesh_ends)
            self._c_tri_mesh_ends = np.array(
                self._tri_mesh_ends, dtype=np.uint)
            ...
            self._c_sphere_mesh_ends = np.array(
                self._sphere_mesh_ends, dtype=np.uint32)
            ...
            self._free_bvhtrees()
            ctracer.bvhtree_triangles_builds(ct._c_geometries)
            self._c_tri_bvhtree = True
            self._c_sphere_bvhtree = True
            ctracer.bvhtree_spheres_build(ct._c_geometries)
            self._state = "geometry_loaded"
        if(self._state == "geometry_loaded"):
            ...
            for i, inst in enumerate(self._created_instances):
                ...
                py_trans_p = inst.transformation.ctypes.data_as(
            ...
            self._c_instances_bvhtree = True
            ctracer.bvhtree_instances_build(
                self._c_geometris)
