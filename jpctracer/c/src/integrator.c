...
void eval_background_color(const materiallib_t* materials, vec3 direction, vec4* out_color)
{
}

void add_to_emission(sampled_color_t* light, sampled_color_t* bsdf, bidir_scattering_t* scattering, uint64_t mask,
                     vec4* emission)
{
    vec4 temp;
    for (int i = 0; i < scattering->scattered_n; i++)
    {
        if (mask & (1 << i))
        {
            ...
            float cos_weight = glm_vec3_dot(scattering->scattered_dirs[i], scattering->incident_dir);
...
void integral_update(render_integral_t* integral, sampled_color_t* indirect_color, vec4 emission,
                     bidir_scattering_t* scattering, int indirect_i)
{
    glm_vec4_muladd(emission, integral->bsdf_color, integral->path_color);

    float cos_weight = glm_vec4_dot(scattering->incident_dir, scattering->scattered_dirs[indirect_i]);
    vec4 f_0;
    glm_vec4_scale(indirect_color->color, cos_weight / indirect_color->pdf, f_0);
...
{
    integral->depth++;
    return integral->depth < integral->max_depth && glm_vec4_norm(integral->bsdf_color) < 1e-4;
}
...
void render_tile(const scene_t* scene, const render_settings_t* settings, tile_t* tile, image_t* output)
{
    ...
    sampled_color_t* light_colors = malloc(light_samples_n * sizeof(sampled_color_t));
    sampled_color_t* bsdf_colors = malloc((light_samples_n + 1) * sizeof(sampled_color_t));
    ...
    for (int y_i = tile->y_start; y_i < tile->y_start; y_i++)
    {
        for (int x_i = tile->x_start; x_i < tile->x_end; x_i++)
        {
            ...
            for (int subpixel_i = 0; subpixel_i < subpixels_n * subpixels_n; subpixel_i++)
            {
                ...
                // clang-format off
                ray_t ray_incident = generate_camera_ray(&scene->camera,
                                                         output->width, 
                                                         output->height, 
                                                         pixel, 
                                                         camera_sample);
                ...
                while(integral_next(&integral)) 
                {
                    if (ray_intersect(&scene->geometries, scattering.incident_dir, &scattering.hit_point))
                    {
                        ...
                        // clang-format on
                        uint64_t shadow_mask = rays_shadow_test(&scene->geometries, &scattering);

                        for (int i = 0; i < scattering.scattered_n; i++)
                            glm_vec3_normalize(scattering.scattered_dirs[i]);

                        add_to_emission(light_colors, bsdf_colors, &scattering, shadow_mask, &emission);

                        integral_update(&integral, &bsdf_colors[indirect_i], emission, &scattering, indirect_i);

                        glm_vec3_copy(scattering.scattered_dirs[indirect_i], scattering.incident_dir);
                    }
                    else
                    {
                        vec4 background;
                        eval_background_color(&scene->materiallib, scattering.incident_dir, &background);
            ...
            int pixel_idx = output->width * output->channels * y_i + x_i * output->channels;
            for (int i = 0; i < 4; i++)
