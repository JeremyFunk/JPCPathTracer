...
struct diffuse_params
{
    ...
    {
        vec4 luminance;
        glm_vec4_scale(color, (float)(1./M_PI), luminance);
        luminance[3] = 1;
        for (uint i = 0; i < n; i++)
        {
            if (same_hemisphere_nspace(incident_dir, scattered_dirs[i]))
            {
                ...
                out_colors[i].pdf = (float)(std::fabs(cos_theta(scattered_dirs[i])) / M_PI);

                glm_vec3_copy(luminance, out_colors[i].color);

            }
            else
            {
                out_colors[i].pdf = 1;
                glm_vec3_zero(out_colors[i].color);
            }
