...
namespace cameras_test {

    TEST(camera_test,ProjectionCameraRayTest3x3)
    {
        auto sampler = std::make_shared<samplers::DebugSampler>();
        auto camera = std::make_shared<cameras::ProjectionCamera>(3,3,1);

        for(int x = 0; x < 3; x++){
            for(int y = 0; y < 3; y++){
                auto ray = camera->GenerateRay(sampler, {x,y});
                utilities::PrintRay(ray);
            }    
            std::cout<<"\n";
        }
    }
    ...
    {
        auto sampler = std::make_shared<samplers::DebugSampler>();
        auto camera = std::make_shared<cameras::ProjectionCamera>(3,4,1);
    ...
    {
        auto sampler = std::make_shared<samplers::DebugSampler>();
        auto camera = std::make_shared<cameras::ProjectionCamera>(4,3,1);
