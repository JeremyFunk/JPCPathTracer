...
namespace jpctracer
{
    ...
    inline Vec3 SphericalDirection(Prec sin_theta,Prec cos_theta, Prec phi )
    {
        return Vec3({sin_theta*std::cos(phi),sin_theta*std::sin(phi),cos_theta});
    }
    inline Vec3 SphericalDirection(Prec theta, Prec phi)
    {
        return SphericalDirection(std::sin(theta),std::cos(theta),phi);
    }

    inline Prec SphericalTheta(const Vec3& vec)
    {
        return std::acos(std::clamp<Prec>(vec[2],-1,1));
    }
    inline Prec SphericalPhi(const Vec3& vec)
    {
        Prec p = std::atan2(vec[1],vec[0]);
        return (p<0) ? (p+2*M_PI) : M_PI;
    }
    
    inline Prec CosTheta(const Vec3 &v) 
    { 
        return v[2]; 
    }

    inline Prec Cos2Theta(const Vec3 &v) 
    { 
        return v[2] * v[2]; 
    }

    inline Prec AbsCosTheta(const Vec3& v) 
    { 
        return std::abs(v[2]); 
    }
    ...
    inline Prec SinTheta(const Vec3& v) 
    { 
        return std::sqrt(Sin2Theta(v)); 
    }

    inline Prec TanTheta(const Vec3& v) 
    { 
    ...
    inline Prec Cos2Phi(const Vec3& v) 
    { 
        return CosPhi(v) * CosPhi(v); 
    }

    inline Prec Sin2Phi(const Vec3& v) 
    { 
    ...
    {
        return  -1 * v1 + 2 * v1.dot(normal) * normal;
    }
}