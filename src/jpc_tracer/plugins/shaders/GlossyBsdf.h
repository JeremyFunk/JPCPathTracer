...
inline Prec BeckmanDistribution(Vec3 microfacet_normal, Prec alpha2)
{
    ...
    return GreaterZero(microfacet_normal[2]) * std::exp(-tan2 / alpha2) / (M_PI * alpha2 * cos2 * cos2);
};
// Microfacet Models for Refraction through Rough Surfaces
inline Prec BeckmanShadowSingle(Vec3 v, Vec3 microfacet_normal, Prec alpha)
{
    return GreaterZero(v.dot(microfacet_normal) / v[2]) * alpha < 1.6
               ? 3.535 * alpha + 2.181 * alpha * alpha / (1 + 2.276 * alpha + 2.577 * alpha * alpha)
               : 1;
}
// Pbrt
inline Prec BeckmannLambda(Prec alpha, const Vec3& direction)
{
    ...
    return a < 1.6 ? (1 - 1.259f * a + 0.396f * a * a) / (3.525f * a + 2.181f * a * a) : 0;
}
...
inline BsdfNode GlossyBsdf(Ray scattered, Spectrum color, Prec roughness)
{
    return CreateBsdf(MATERIAL_GLOSSY, GlossyBsdfClosure(color, roughness, scattered));
}
