...
int number_leading_zeros(uint32_t* morton, const int& idx, const int offset, const size_t& number_nodes)
{
    ...
    if (second_idx < 0 || second_idx > number_nodes)
        return -1;
...
{
    return (number_leading_zeros(morton, idx, 1, number_nodes) - number_leading_zeros(morton, idx, -1, number_nodes) <
            0)
               ? -1
               : 1;
}
                    ...
                    const size_t& number_nodes)
{
    ...
    do
    {
        ...
        if (new_split_idx < max_idx &&
            number_leading_zeros(morton, min_idx, new_split_idx - min_idx, number_nodes) > compare_number)
        {
...
{
    // a = a & b
    a.Max = {std::max(a.Max[0], b.Max[0]), std::max(a.Max[1], b.Max[1]), std::max(a.Max[2], b.Max[2])};

    a.Min = {std::min(a.Min[0], b.Min[0]), std::min(a.Min[1], b.Min[1]), std::min(a.Min[2], b.Min[2])};
}
...
{
    Bounds3D bound_out = bounds[min_idx];

    for (uint idx = min_idx + 1; idx <= max_idx; idx++)
        Union(bound_out, bounds[idx]);
