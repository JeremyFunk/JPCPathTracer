...
Ref<std::vector<Ref<IShape>>> generate_shapes() 
{
    ...
    auto material_white_glossy = MakeRef<GlossyMaterial>(color_white,0.05);
...
Ref<std::vector<Ref<ILight>>> generate_lights() 
{
    ...
    lightList->push_back(MakeRef<PointLight>(Vec3(0, 0, -7), RGBSpectrum::FromRGB({1000,1000,1000})));
