...
std::shared_ptr<std::vector<std::shared_ptr<IShape>>> generate_shapes() 
{
    ...
    auto material_white_glossy = std::make_shared<GlossyMaterial>(color_white,0.);
    ...
    //Transformation monkey_transformation(Vec3(0, 0, -5), Vec3(0, 0, 0), Vec3(.8,.8,.8));
    //auto monkeyMesh = LoadMesh(MonkeyPath(), material_white_glossy, std::make_shared<Transformation>(monkey_transformation), shapeList);
...
std::shared_ptr<std::vector<std::shared_ptr<ILight>>> generate_lights() 
{
    ...
    lightList->push_back(std::make_shared<PointLight>(Vec3(-1, 0, -7), RGBSpectrum::FromRGB({2000,2000,2000})));
